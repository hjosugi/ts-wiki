/**
 * PostgreSQL page-write repository contract tests — integration.
 ...
 * verified without a real search backend (the tsvector indexer is a later
 * slice).
 */
...
describe.skipIf(!testPostgresUrl)('postgres page-write contracts', () => {
  let harness: PostgresContractDb
  beforeAll(async () => { harness = await createPostgresContractDb('kw_page_write_contract') })
  beforeEach(async () => { await harness.reset() })
  afterAll(async () => { await harness?.close() })

  test('create: persists page + revision, clears redirects, indexes, and guards duplicate paths', async () => {
    const spy = makeSpy()
    const write = createPostgresPageWriteRepository(harness.db, spy.indexer)
  ...
  test('remove: soft-deletes, drops redirects, and removes from the index', async () => {
    ...
    expect(spy.removed).toEqual(['p1'])
    const read = createPostgresPageReadRepository(harness.db)
