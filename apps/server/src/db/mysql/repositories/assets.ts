...
/** MySQL implementation of the driver-neutral asset contract. */
export const createMysqlAssetRepository = (db: MysqlDb): AssetRepository => ({
  ...
  async insertReferences(pageIds, assetId) {
    if (!pageIds.length) return
    await db.insert(pageAssetRefs).ignore().values(pageIds.map((pageId) => ({ pageId, assetId })))
  },
