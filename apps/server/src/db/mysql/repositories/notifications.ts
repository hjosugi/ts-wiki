...
/** MySQL implementation of the driver-neutral notification contract. */
export const createMysqlNotificationRepository = (db: MysqlDb): NotificationRepository => ({
  ...
  async setWatching(userId, path, watching, createdAt) {
    if (watching) {
      await db.insert(pageWatchers).ignore().values({ userId, path, createdAt })
      return
    }
  ...
  async moveWatchers(fromPath, toPath) {
    await db.transaction(async (tx) => {
      ...
      for (const watcher of watchers) {
        await tx.insert(pageWatchers).ignore().values({ ...watcher, path: toPath })
      }
