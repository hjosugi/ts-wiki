/**
 * MySQL auth/identity repository contract tests — integration. Env-gated.
 *
 * Mirrors the Postgres auth contracts (`../postgres/repositories-auth.test.ts`)
 * so behaviour stays identical across drivers, isolated in its own database.
 */
import { afterAll, beforeAll, beforeEach, describe, expect, test } from 'bun:test'
import { users } from './schema.ts'
import { createMysqlContractDb, testMysqlUrl, type MysqlContractDb } from './test-support.ts'
import { createMysqlUserRepository } from './repositories/users.ts'
import { createMysqlAuthAccountRepository } from './repositories/auth-accounts.ts'
import { createMysqlAuthRecoveryRepository } from './repositories/auth-recovery.ts'
import { createMysqlPasskeyRepository } from './repositories/passkeys.ts'
import { createMysqlTotpRepository } from './repositories/totp.ts'
import { createMysqlApiKeyRepository } from './repositories/api-keys.ts'
...
describe.skipIf(!testMysqlUrl)('mysql auth repository contracts', () => {
  let harness: MysqlContractDb
  const seedUser = (over: Partial<UserRecord> = {}) => harness.db.insert(users).values(makeUser(over))

  beforeAll(async () => {
    harness = await createMysqlContractDb('kw_auth_contract')
  }, 30_000)
  beforeEach(async () => {
    await harness.reset()
  }, 30_000)
  ...
  test('users: count, duplicate-email guard, find, and update', async () => {
    const repo = createMysqlUserRepository(harness.db)
  ...
  test('auth accounts: create-with-account, linked lookup, and relink', async () => {
    const repo = createMysqlAuthAccountRepository(harness.db)
  ...
  test('auth recovery: password reset + email verification lifecycle', async () => {
    ...
    const repo = createMysqlAuthRecoveryRepository(harness.db)
  ...
  test('passkeys: crud, optimistic counter update, and challenge lifecycle', async () => {
    const repo = createMysqlPasskeyRepository(harness.db)
    const passkey = {
  ...
  test('totp: secret, enable with recovery codes, consume, replace, and disable', async () => {
    await seedUser({ id: 'u1', email: 'a@x' })
    const repo = createMysqlTotpRepository(harness.db)
    const users_ = createMysqlUserRepository(harness.db)
  ...
  test('api keys: ordering, duplicate hash, revoke, and markUsedIfActive', async () => {
    const repo = createMysqlApiKeyRepository(harness.db)
