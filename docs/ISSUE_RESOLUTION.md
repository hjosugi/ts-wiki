...
| #34 Templates / starter pages | Covered by the built-in create-page templates in `PageEdit.vue`: blank, decision, how-to, meeting notes, and spec. They prefill title, path, content, and can be combined with labels/status metadata. Custom persisted templates stay out until users need them. |
